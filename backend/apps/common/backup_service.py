...
def _snapshot_payload() -> dict:
    ...
    payload = {
        ...
        'competitions': list(
            Competition.objects.filter(project_id__in=project_ids).values(
                'project__code', 'name', 'comp_type', 'level', 'organizer',
        ...
        'finance_expenses': list(
            FinanceExpense.objects.filter(project_id__in=project_ids).values(
                'project__code', 'title', 'amount', 'expense_date',
        ...
        'finance_incomes': list(
            FinanceIncome.objects.filter(project_id__in=project_ids).values(
                'project__code', 'title', 'amount', 'income_type',
        ...
        'files': list(
            FileAsset.objects.filter(project_id__in=project_ids).values(
                'project__code', 'name', 'file', 'level', 'size',
        ...
        'file_versions': list(
            FileVersion.objects.filter(
                ...
                'file_asset__project__code', 'file_asset__name', 'version',
    ...
    related_models = {
        ...
        'competition': (
            ...
            lambda obj: {'project': obj.project.code, 'name': obj.name},
        ),
        'finance_expense': (
...
def _upsert_snapshot_row(
    ...
    *,
    ...
    timestamps = {}
    for field_name in ('created_at', 'updated_at', 'filled_at', 'started_at'):
...
def _restore_snapshot_overlay(snapshot: dict) -> dict:
    ...
    competition_keys = {(row['project__code'], row['name']) for row in competition_rows}
    _delete_missing(
        Competition.objects.filter(project__code__in=project_codes).select_related('project'),
        competition_keys,
        lambda obj: (obj.project.code, obj.name),
        stats,
    )
    ...
    for row in income_rows:
        ...
        _upsert_snapshot_row(
            ...
            key_lookup,
            row,
            relations={'recorded_by': _optional_id(User, {'email': row.get('recorded_by__email')})},
            stats=stats,
        )

    file_rows = snapshot.get('files', [])
    file_keys = {(row['project__code'], row['name']) for row in file_rows}
    _delete_missing(
        FileAsset.objects.filter(project__code__in=project_codes).select_related('project'),
        file_keys,
        lambda obj: (obj.project.code, obj.name),
        stats,
        file_fields=('file',),
    ...
    version_keys = {
        (row['file_asset__project__code'], row['file_asset__name'], row['version'])
        for row in version_rows
    }
    _delete_missing(
        FileVersion.objects.filter(file_asset__project__code__in=project_codes).select_related(
            'file_asset', 'file_asset__project'
        ),
        version_keys,
        lambda obj: (obj.file_asset.project.code, obj.file_asset.name, obj.version),
        stats,
        file_fields=('file',),
    )
    for row in version_rows:
...
def _resolve_related_object_id(row: dict):
    ...
    if related_type == 'competition':
        from apps.competitions.models import Competition
        return _optional_id(Competition, {'project__code': key.get('project'), 'name': key.get('name')})
