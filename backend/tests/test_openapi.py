...
class TestOpenAPISchema:
    ...
    def test_schema_returns_openapi_version(self, teacher_client):
        """返回 openapi 版本字段"""
    ...
    def test_schema_has_info(self, teacher_client):
        """返回 info 字段"""
        data = extract_data(teacher_client.get('/api/v1/common/openapi/schema/'))
        assert 'info' in data
        assert 'title' in data['info']
        assert 'version' in data['info']

    def test_schema_has_paths(self, teacher_client):
        """返回 paths 字段且非空"""
        data = extract_data(teacher_client.get('/api/v1/common/openapi/schema/'))
    ...
    def test_schema_endpoints_count(self, teacher_client):
        """返回 endpoints_count"""
        data = extract_data(teacher_client.get('/api/v1/common/openapi/schema/'))
        assert 'endpoints_count' in data
        assert data['endpoints_count'] > 0

    def test_schema_path_has_methods(self, teacher_client):
        """至少一个路径包含 HTTP 方法"""
        data = extract_data(teacher_client.get('/api/v1/common/openapi/schema/'))
        has_method = any(
...
class TestAPIEndpointList:
    ...
    def test_endpoints_returns_list(self, teacher_client):
        """返回端点列表"""
    ...
    def test_endpoints_only_api_paths(self, teacher_client):
        """仅返回 API 路径"""
        data = extract_data(teacher_client.get('/api/v1/common/openapi/endpoints/'))
        for ep in data['endpoints']:
            assert 'api' in ep['path']

    def test_endpoints_have_methods(self, teacher_client):
        """每个端点包含 methods 字段"""
        data = extract_data(teacher_client.get('/api/v1/common/openapi/endpoints/'))
        for ep in data['endpoints']:
