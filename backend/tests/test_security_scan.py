...
class TestSecurityScan:
    ...
    def test_scan_returns_checks(self, teacher_client):
        """返回安全检查清单"""
        resp = teacher_client.get('/api/v1/common/security-scan/')
    ...
    def test_scan_includes_secret_key_check(self, teacher_client):
        """包含 SECRET_KEY 检查项"""
        data = extract_data(teacher_client.get('/api/v1/common/security-scan/'))
        items = [c['item'] for c in data['checks']]
        assert 'secret_key_from_env' in items

    def test_scan_includes_debug_check(self, teacher_client):
        """包含 DEBUG 检查项"""
        data = extract_data(teacher_client.get('/api/v1/common/security-scan/'))
        items = [c['item'] for c in data['checks']]
    ...
    def test_scan_includes_https_check(self, teacher_client):
        """包含 HTTPS 检查项"""
        data = extract_data(teacher_client.get('/api/v1/common/security-scan/'))
        items = [c['item'] for c in data['checks']]
        assert 'https_recommended' in items

    def test_scan_includes_password_policy(self, teacher_client):
        """包含密码策略检查项"""
        data = extract_data(teacher_client.get('/api/v1/common/security-scan/'))
        items = [c['item'] for c in data['checks']]
        assert 'password_policy' in items

    def test_scan_includes_cors_check(self, teacher_client):
        """包含 CORS 检查项"""
        data = extract_data(teacher_client.get('/api/v1/common/security-scan/'))
        items = [c['item'] for c in data['checks']]
        assert 'cors_restricted' in items

    def test_scan_returns_score(self, teacher_client):
        """返回安全评分"""
        data = extract_data(teacher_client.get('/api/v1/common/security-scan/'))
        assert 'score' in data
        assert 0 <= data['score'] <= 100

    def test_scan_each_check_has_severity(self, teacher_client):
        """每项检查包含严重级别"""
        data = extract_data(teacher_client.get('/api/v1/common/security-scan/'))
