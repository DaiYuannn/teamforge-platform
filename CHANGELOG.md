...
- 新增任务协作详情，将子任务、依赖关系、评论、回复和成员提及集中到单个任务工作区。
- 新增项目协作工作台，覆盖里程碑、风险、讨论、知识、复盘、模板实例化以及风险、健康度、复盘和材料分析。
- 补齐文件全生命周期：项目文件夹、标签、移动、版本、分享、软删除、回收站和 DOCX/XLSX/PPTX 有界只读预览。
- 新增分析工作台，统一自定义看板、自定义报表生成、智能周报和 Markdown 导出。
- 新增平台能力中心，集中管理自定义角色、团队、审批流、自定义表单、外部平台和 Git 仓库。
- 新增工程控制台，展示真实请求与 SQL 形状采样、OpenAPI 索引和 API/浏览器质量门禁。
- 新页面已接入响应式导航和路由，并保留项目范围、外部协作者及系统管理员的访问边界。

### 用户体验与个性化

- 常用入口升级为账户级配置：用户可从当前角色和成员状态有权访问的导航中选择、调整顺序或移除入口，最多保存 8 个，并在首页与桌面侧栏同步展示。
- 登录、刷新与账户切换时分别加载当前账户的常用入口；不可访问或已失效的入口会被权限过滤，不会借助收藏绕过导航边界。
- 新增账户级浅色、深色、跟随系统和定时夜间模式；定时模式支持跨午夜时段，并在边界时间及系统配色变化时自动刷新。
- 桌面顶栏和移动顶栏新增浅色/深色快捷切换按钮；主题偏好与账户主色共同持久化，登录、刷新、账户切换和退出时同步应用或恢复默认系统模式。
- 快捷切换保存失败时恢复账户原主题；偏好页仍完整保留浅色、深色、跟随系统和定时四种模式及定时时段配置。
- 暗色界面覆盖 Element Plus、自定义业务页面和 ECharts 图表，并为文字强调色与白字填充主色分别保证可读性。
- “记住我”现在同时控制服务端刷新令牌期限和浏览器存储范围：普通登录仅保留当前会话，勾选后可持久化 30 天。
- 忘记密码接入防账户枚举的一次性邮件令牌、独立重置页和密码强度校验，令牌使用后立即失效。
- 新增中英文运行时词典、登录页语言切换、全局桌面/移动导航翻译和账户级语言持久化；Django LocaleMiddleware 已启用。

### 平台能力闭环

- 自定义角色权限按项目范围约束项目、任务、经费、成员和报表的查看/创建/管理动作，授权项目与未授权项目保持隔离。
- 默认自定义看板现在决定首页组件顺序和项目过滤；分析工作台可打开真实指标、明细并跳转业务详情。
- 汇总、对比、趋势报表具有独立数据语义，表格、柱状、折线和饼图按明确数值字段真实渲染。
- 外部平台使用有界 HTTP 检测与拉取，Git 使用 `ls-remote` 校验分支并同步提交 SHA；凭据以 Fernet 密文保存且接口不回传。
- 通用审批通过结构化元数据原子联动请假、报销、敏感资料访问和项目变更；自定义流程类型仍可作为纯通用审批使用。
- 自定义表单定义和提交均执行服务端结构校验，拒绝未知字段并验证必填、类型、选项、长度、范围和正则规则。

### API、数据与工程质量

- OpenAPI 统一为 3.0.3 契约，补齐 Bearer JWT、请求/响应 serializer、二进制下载、SSE、ICS、ZIP、Office 和报表媒体类型。
- API 文档入口统一为机器可读 schema 和端点索引，撤下已失效的 Swagger/Redoc 链接。
- 文件模块新增文件夹和软删除字段迁移；正式部署需按常规流程执行迁移。
- 用户偏好新增 `theme_mode`、`schedule_start`、`schedule_end`，正式部署需应用 `users.0007_userpreference_schedule_end_and_more` 迁移。
- 用户偏好新增 `language`，正式部署需应用 `users.0008_userpreference_language` 迁移；密码重置部署需配置 `FRONTEND_URL` 和 SMTP 环境变量。
- CI 配置已覆盖后端 lint、系统检查、迁移漂移、OpenAPI、pytest，以及前端依赖审计、lint、类型检查、测试、构建和桌面/移动浏览器流程。
- 新增路由契约、文件闭环、授权边界、性能采样、OpenAPI 和 axe-core 无障碍回归。

### P1/P2 修复

- 全局搜索复用项目与文件可见范围，根项目纳入项目范围判断，敏感文件不会通过搜索结果泄露元数据；`limit` 仅接受 1 至 20。
- 统一任务、文件、比赛、知识、讨论和项目工作台深链参数，前端完整呈现七类搜索结果，并在导航时保留既有查询参数。
- 外部协作者和已退出账号不再显示不可用的全局搜索；搜索失败提供可见反馈。
- 分享链接访问/下载计数及敏感资料审批/驳回采用事务行锁，避免并发更新丢失和重复审批。
- 搜索结果、讨论、知识和移动归档卡片补齐 Enter/Space 操作、焦点样式及按钮语义。
- 根 Compose 健康检查统一到 `/api/v1/common/health/`，新增 Docker 环境示例与备份持久卷；后端镜像及 CI 统一到 Python 3.10 系列。

### 发布验收状态

> 以下结论来自 2026-07-27 本地统一版本环境的实际退出状态；不据此推断远程 CI 或生产环境已经通过。

- 后端：统一为 Python 3.10.20、Django 5.0.6、DRF 3.15.1；flake8、Django 系统检查和迁移漂移均通过，1371 项 pytest 通过（96.00 秒）。
- 前端：ESLint、类型检查和生产构建通过，41 个测试文件共 204 项 Vitest 通过，构建转换 2678 个模块。
- 端到端：Playwright Chromium `16/16` 通过（桌面 8/8、移动 8/8，51.8 秒），覆盖新增语言持久化与密码重置页。
- 异步环境：最新 Django、Redis、Celery Worker/Beat 已重启；健康接口数据库、缓存、Celery、存储、迁移五项均为 healthy（1 个 Worker），真实 Celery 探针完成并返回 `None`，Beat 成功执行定时报表调度任务（本次无到期报告，`executed=0`）。
- 浏览器：分析工作台、平台能力中心、工程控制台及核心账户、项目、报表、通知、备份流程完成桌面/移动烟测；管理员常用入口排序保存、刷新持久化及恢复原顺序通过，主题 light→dark 保存、刷新持久化及恢复 light 通过；390x844 移动端无横向溢出，主题按钮与 4 个常用入口正常。
- 产品边界：OAuth 能力维持未配置预留，本版本不新增实现或启用配置。
- 外部环境：本节不声明远程 CI、生产服务器、生产 Worker/Beat、真实第三方凭据、提交或版本标签已验证。
